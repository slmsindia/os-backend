...
const hierarchyController = {
  ...
  getDescendants: async (req, res) => {
    ...
    const { identity, status, search, page = 1, limit = 20, startDate, endDate, parentId } = req.query;
    ...
    try {
      ...
      users.forEach(u => {
        if (u.path) {
          u.path.split('/').forEach(id => { if (id && id.length > 5) allPathIds.add(id); });
        }
      });
      ...
      const userMap = {};
      pathUsers.forEach(u => userMap[u.id] = u);
