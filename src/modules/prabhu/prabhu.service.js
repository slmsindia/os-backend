...
const SEND_ENDPOINT_PATHS = {
  ...
  GetServiceChargeByCollection: '/Send/GetServiceChargeByCollection',
  CancelTransaction: '/Send/CancelTransaction',
  UnverifiedTransactions: '/Send/UnverifiedTransactions',
  ComplianceTransactions: '/Send/ComplianceTransactions',
  ...
  ValidateBankAccount: '/Send/ValidateBankAccount',
  CreateReceiver: '/Send/CreateReceiver',
  CreateCustomer: '/Send/CreateCustomer',
  GetUnverifiedCustomers: '/Send/GetUnverifiedCustomers',
  GetCustomerByMobile: '/Send/GetCustomerByMobile',
  RegisterComplaint: '/Send/RegisterComplaint',
  TrackComplaint: '/Send/TrackComplaint'
};

const buildSendPathCandidates = (endpointPath) => {
  const normalized = String(endpointPath || '').trim();
  if (!normalized) return [];

  const normalizedWithSlash = normalized.startsWith('/') ? normalized : `/${normalized}`;
  const directSend = normalizedWithSlash.replace(/^\/api\/Send\//i, '/Send/');
  const apiSend = normalizedWithSlash.replace(/^\/Send\//i, '/api/Send/');

  const preferred = String(process.env.PRABHU_SEND_PATH_PREFIX || 'auto').toLowerCase();
  if (preferred === 'api') {
    return [...new Set([apiSend, directSend])];
  }
  if (preferred === 'send') {
    return [...new Set([directSend, apiSend])];
  }
...
const ensureConfigured = () => {
  ...
  if (config.sendMode === 'soap' && !config.wsdlUrl) {
    throw new Error('PRABHU SOAP WSDL missing. Set PRABHU_SEND_WSDL for soap mode.');
  }
  return config;
};
...
const withSendDefaults = (payload, config) => {
...
const callSendRestEndpoint = async (operation, payload = {}, context = {}) => {
  ...
  if (!endpointPath) {
    throw new Error(`Unsupported PRABHU REST operation: ${operation}`);
  }
  const method = 'POST';
...
const callSoapEndpoint = async (operation, payload = {}, context = {}) => {
