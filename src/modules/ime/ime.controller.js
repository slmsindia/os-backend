...
const imeEKycController = require('./ime.ekyc.controller');
...
const getStaticData = async (req, res) => {
  try {
    ...
    console.log(`[IME Static Data] Type: ${type}, Reference: ${reference || ''}`);
    const result = await imeService.getStaticData(type, reference || '');
    console.log(`[IME Static Data] Response keys:`, Object.keys(result || {}));
    console.log(`[IME Static Data] Full response:`, JSON.stringify(result, null, 2).slice(0, 1000));
    
    return ok(res, 'Static data retrieved', result);
  } catch (error) {
    console.error('[IME Static Data] Error:', error.message);
    return fail(res, error);
  }
};
...
/**
 * SOA Report - Statement of Account
 * GET /api/ime/reports/soa?fromDate=YYYY/MM/DD&toDate=YYYY/MM/DD
 */
const soaReport = async (req, res) => {
  try {
    const { fromDate, toDate } = req.query;

    if (!fromDate || !toDate) {
      return badRequest(res, 'fromDate and toDate are required', ['fromDate', 'toDate']);
    }

    // Validate date format
    const dateFormatRegex = /^\d{4}\/\d{2}\/\d{2}$/;
    if (!dateFormatRegex.test(fromDate) || !dateFormatRegex.test(toDate)) {
      return badRequest(res, 'Dates must be in YYYY/MM/DD format');
    }

    const result = await imeService.soaReport(fromDate, toDate);
    return ok(res, 'SOA report retrieved', result);
  } catch (error) {
    return fail(res, error);
  }
};
...
module.exports = {
  ...
  generateOTT: imeEKycController.generateOTT,
  getUniqueId: imeEKycController.getUniqueId,
  bioKyc: imeEKycController.bioKyc,
  customerOnboarding: imeEKycController.customerOnboarding,
  customerRequeryEKyc: imeEKycController.customerRequery,
  checkEntityStatus: imeEKycController.checkEntityStatus,
  aadharCustomerRegistration: imeEKycController.aadharCustomerRegistration,
  aadharEntityReprocess: imeEKycController.aadharEntityReprocess
};
